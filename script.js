...
const CONFIG={
  ...
  method:2,
  vricUrl:'https://vric.org/prayertimes/',
  vricSyncMinutes:360,
...
async function syncVricTimes(showSuccess){
  const raw=await fetchTextWithFallback(CONFIG.vricUrl);
  const parsed=parseVricPrayerTimes(raw);
  ...
  lastVricSyncStatus='VRIC live sync: '+stamp;
  try{
    localStorage.setItem('aslima_vric_cache',JSON.stringify({timings,jumuah:CONFIG.jumuah,stamp}));
  }catch(e){}

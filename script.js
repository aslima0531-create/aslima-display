...
const AZAAN_VOICES={
  makkah:{id:'makkah',name:'Makkah Adhan',short:'Makkah',url:'https://praytimes.org/audio/sunni/Adhan-Makkah.mp3',note:'Masjid al-Haram style'},
  madinah:{id:'madinah',name:'Madinah Adhan',short:'Madinah',url:'https://praytimes.org/audio/sunni/Adhan-Madinah.mp3',note:'Prophet’s Mosque style'},
  abdulbasit:{id:'abdulbasit',name:'Abdul Basit',short:'Abdul Basit',url:'https://praytimes.org/audio/sunni/Abdul-Basit.mp3',note:'Renowned Egyptian voice'},
  minshawi:{id:'minshawi',name:'Minshawi',short:'Minshawi',url:'https://praytimes.org/audio/sunni/Minshawi.mp3',note:'Classic Egyptian maqam'},
  yusufislam:{id:'yusufislam',name:'Yusuf Islam',short:'Yusuf Islam',url:'https://praytimes.org/audio/sunni/Yusuf-Islam.mp3',note:'Recognizable English-era recording'}
};
const AZAAN_VOICE_ORDER=['makkah','madinah','abdulbasit','minshawi','yusufislam'];
let selectedAzaanVoice=localStorage.getItem('aslima_azaan_voice') || 'madinah';
...
function getAzaanVoice(id){
  return AZAAN_VOICES[id] || AZAAN_VOICES.madinah;
}
let azaanVoiceLoadPromise=Promise.resolve();
function updateAzaanAudioSource(){
  const voice=getAzaanVoice(selectedAzaanVoice);
  if(!audio)return Promise.reject(new Error('Audio player missing'));
  if(audio.dataset.voiceId===voice.id && audio.readyState>=2)return Promise.resolve(voice);
  ...
  audio.dataset.voiceId=voice.id;
  audio.preload='auto';
  azaanVoiceLoadPromise=new Promise((resolve,reject)=>{
    let done=false;
    const finish=(ok,err)=>{
      ...
      clearTimeout(timer);
      ok?resolve(voice):reject(err||new Error('Audio failed to load'));
    };
    const onReady=()=>finish(true);
    const onError=()=>finish(false,new Error('Could not load '+voice.name));
    const timer=setTimeout(()=>{
      if(audio.readyState>=2)finish(true); else finish(false,new Error('Timed out loading '+voice.name));
    },12000);
    audio.addEventListener('canplay',onReady,{once:true});
    audio.addEventListener('loadedmetadata',onReady,{once:true});
    audio.addEventListener('error',onError,{once:true});
    try{audio.load();}catch(e){finish(false,e);}
  });
  return azaanVoiceLoadPromise;
}
async function ensureSelectedAzaanVoiceReady(){
  const voice=getAzaanVoice(selectedAzaanVoice);
  if(audio.dataset.voiceId!==voice.id)updateAzaanAudioSource();
  if(audio.readyState>=2)return voice;
  return azaanVoiceLoadPromise;
}
function renderMuezzinControls(){
  const select=$('drawerMuezzinSelect');
  const now=$('drawerMuezzinNow');
  const voice=getAzaanVoice(selectedAzaanVoice);
  if(select){
    select.innerHTML=AZAAN_VOICE_ORDER.map(id=>{const v=AZAAN_VOICES[id];return `<option value="${v.id}">${v.name}</option>`;}).join('');
    select.value=voice.id;
  }
  if(now)now.textContent='Current voice: '+voice.name+' · '+voice.note;
}
function setAzaanVoice(id,options){
  options=options||{};
  const voice=getAzaanVoice(id);
  selectedAzaanVoice=voice.id;
  try{localStorage.setItem('aslima_azaan_voice',voice.id);}catch(e){}
  updateAzaanAudioSource().catch(()=>{});
  renderMuezzinControls();
  if(!options.quiet)showToast('Muezzin voice: '+voice.name);
  if(options.pushRemote && window.aslimaRemoteRef){
    window.aslimaRemoteRef.update({muezzin:voice.id,updatedAt:new Date().toISOString()}).catch(()=>{});
  }
}
function bindMuezzinControls(){
  const select=$('drawerMuezzinSelect');
  if(select && !select.dataset.bound){
    select.dataset.bound='1';
    select.addEventListener('change',e=>setAzaanVoice(e.target.value,{pushRemote:true}));
  }
  setAzaanVoice(selectedAzaanVoice,{quiet:true});
}
...
function applyAslimaRemoteSettings(data){
  ...
  try{
    ...
    if(data.mode==='manual' && ['Fajr','Dhuhr','Asr','Maghrib','Isha'].every(k=>remoteTimings[k])){
      ...
      lastVricSyncStatus='Manual prayer times active';
      if(window.renderTabletManualTimingUI)window.renderTabletManualTimingUI();
      ...
      saveTimingSourcePrefs();
      lastVricSyncStatus='Phone remote: switching back to VRIC live…';
      // Make the tablet change immediately instead of waiting on a network fetch.
      // First restore the last known VRIC cache, then refresh live VRIC in the background.
      if(wasManual){
        loadCachedVricTimes();
        render();
      }
      loadTimes().then(()=>{
        ...
        lastVricSyncStatus='Phone remote: VRIC refresh failed — using last saved VRIC times';
        loadCachedVricTimes();
        render();
      });
    }
    ...
    if(typeof data.muezzin==='string')setAzaanVoice(data.muezzin,{quiet:true});
    ...
    if(typeof data.muezzin==='string' && data.muezzin.trim()){
      setAzaanVoice(data.muezzin,{quiet:true});
    }
...
async function playAzaan(prayerName,options){
  ...
  try{
    ...
    await ensureSelectedAzaanVoiceReady();
    audio.currentTime=0;
    await audio.play();
...
audio.onerror=()=>{const v=(typeof getAzaanVoice==='function'?getAzaanVoice(selectedAzaanVoice):null);showToast('Azaan audio failed: '+(v&&v.name?v.name:'selected voice'));};

function showAdmin(opts){
  // Deprecated local tablet admin is intentionally disabled.
  // Use admin.html from a phone for all admin changes.
  const adminEl=$('admin');
...
/* ASLIMA V9.1.1 — start Firebase phone remote listener after all app code is loaded */
try{
  if(typeof initAslimaFirebaseRemote === 'function'){
    initAslimaFirebaseRemote();
  }
}catch(e){
  console.warn('Aslima remote init failed', e);
}

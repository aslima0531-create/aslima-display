...
function addMinutesToClock(timeText,delta){
  const base=mins(timeText);
  let total=Math.round(base+delta);
...
function formatRange(start,end){
  return `${to12(start).full} – ${to12(end).full}`;
}

function makroohWindowsForToday(){
  const list=[];
  const hasSunrise=timings && timings.Sunrise;
  if(timings.Fajr && hasSunrise){
    // Fajr is shown as two separate advisory states:
    // 1) After Fajr until sunrise: no voluntary/nafl prayers.
    // 2) Sunrise period: the stricter prayer-restricted window after the sun starts rising.
    list.push({id:'after-fajr',label:'After Fajr',start:timings.Fajr,end:timings.Sunrise,note:'Nafl avoided until sunrise'});
    list.push({id:'sunrise',label:'Sunrise Period',start:timings.Sunrise,end:addMinutesToClock(timings.Sunrise,15),note:'Prayer restricted'});
  }
  if(timings.Dhuhr){
    list.push({id:'zawal',label:'Zawal',start:addMinutesToClock(timings.Dhuhr,-10),end:timings.Dhuhr,note:'Zenith caution'});
  }
  if(timings.Asr && timings.Maghrib){
    // Show the stricter sunset restriction separately and give it priority over
    // the broader After Asr advisory during the final minutes before Maghrib.
    list.push({id:'sunset',label:'Sunset Period',start:addMinutesToClock(timings.Maghrib,-10),end:timings.Maghrib,note:'Prayer restricted',priority:3});
    list.push({id:'after-asr',label:'After Asr',start:timings.Asr,end:timings.Maghrib,note:'Nafl avoided until Maghrib',priority:1});
  }
  return list;
}

function minutesUntilWindow(now,start){
  let d=mins(start)-now;
  if(d<0)d+=1440;
  return d;
}

function renderMakroohWindows(){
  const box=$('makrooh');
  const listEl=$('makroohList');
  const statusEl=$('makroohStatus');
  const titleEl=$('makroohTitle');
  if(!box || !listEl || !statusEl)return;
  const d=new Date();
  const now=d.getHours()*60+d.getMinutes()+d.getSeconds()/60;
  const windows=makroohWindowsForToday();
  if(!windows.length){
    box.classList.remove('is-active');
    box.classList.add('is-idle');
    if(titleEl)titleEl.textContent='Prayer Advisory';
    statusEl.classList.remove('active');
    statusEl.textContent='Waiting';
    listEl.innerHTML='<span class="makrooh-chip"><span class="makrooh-chip-main"><b>Times loading</b></span><small>Advisory appears after prayer times load.</small></span>';
    return;
  }
  const active=windows
    .filter(w=>now>=mins(w.start) && now<mins(w.end))
    .sort((a,b)=>(b.priority||2)-(a.priority||2))[0];
  const nextWin=windows.slice().sort((a,b)=>minutesUntilWindow(now,a.start)-minutesUntilWindow(now,b.start))[0];
  const item=active || nextWin;
  const isRestricted=item && (item.id==='sunrise' || item.id==='sunset' || item.id==='zawal');
  box.classList.toggle('is-active',!!active);
  box.classList.toggle('is-idle',!active);
  statusEl.classList.toggle('active',!!active);
  if(titleEl)titleEl.textContent=active && isRestricted ? 'Prayer Restricted' : 'Prayer Advisory';
  if(active){
    statusEl.textContent='Active now';
  }else if(item){
    const eta=Math.round(minutesUntilWindow(now,item.start));
    statusEl.textContent=`Next in ${eta<60?eta+'m':Math.floor(eta/60)+'h '+String(eta%60).padStart(2,'0')+'m'}`;
  }
  const note = active ? item.note : `Upcoming: ${item.note}`;
  listEl.innerHTML=`<span class="makrooh-chip ${active?'active':'next'}"><span class="makrooh-chip-main"><b>${item.label}</b><span class="range">${formatRange(item.start,item.end)}</span></span><small>${note}</small></span>`;
}

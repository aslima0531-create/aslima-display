...
function bindAzaanDrawer(){
  ...
  tab.addEventListener('pointermove',e=>{
    ...
    if(dx<-26)openAzaanDrawer();
    if(dx>26)closeAzaanDrawer();
  });
  tab.addEventListener('pointerup',e=>{
    if(!dragging)return;
    const dx=e.clientX-startX;
    dragging=false;
    if(Math.abs(dx)<8)drawer.classList.toggle('open');
  });
  tab.addEventListener('keydown',e=>{
    if(e.key==='Enter' || e.key===' '){e.preventDefault();drawer.classList.toggle('open');}

...
test('phone uses the tablet resolved Azaan timings as its authoritative schedule',()=>{
  ...
  assert.match(html,/register\('\.\/sw\.js\?v=996'\)/);
  assert.match(admin,/register\('\.\/sw\.js\?v=996'\)/);
  assert.match(serviceWorkerSource,/const VERSION='996'/);
});
...
test('v996 service-worker upgrade removes older caches and precaches runtime, Focus, discovery, and audio assets',async()=>{
